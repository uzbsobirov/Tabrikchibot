...
@dp.callback_query_handler(text="statistika", state='*', user_id=ADMINS)
async def statistika(call: types.CallbackQuery, state: FSMContext):
    await Panel.stat.set()
    count = await db.count_users()
    today = date.today()
    text = f"<b>📆 Bugunki sana: {today}\n\n📊 Bot obunachilari: {count}\n\n⚡️@protabrikbot</b>"
...
async def state_first_sponsor(message: types.Message, state: FSMContext):
    ...
    try:
        if '@' in username:
            await db.add_channel(sponsor=username)
